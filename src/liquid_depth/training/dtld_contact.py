...
class DTLDContactGeometryNet(nn.Module):
    ...
    def forward(
        ...
        return {
            ...
            "curve_confidence": presence_logit.sigmoid()
            * torch.sigmoid(-curve_log_variance),
        }
...
class DTLDContactGeometryLoss(nn.Module):
    ...
    def forward(
        ...
        grid_y, grid_x = torch.meshgrid(y, x, indexing="ij")
        squared_distance = (
            (grid_x[None, None] - target_control[:, :, 0, None, None]).square()
            + (grid_y[None, None] - target_control[:, :, 1, None, None]).square()
        )
